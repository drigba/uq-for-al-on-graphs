...
from graph_al.acquisition.config import AcquisitionStrategyExpectedQueryConfig
...
class AcquisitionStrategyExpectedQuery(AcquisitionStrategyByAttribute):
    
    """ Acquisition strategy that uses approximation to the ground truth uncertainty. 
    
    """
    
    def __init__(self, config: AcquisitionStrategyExpectedQueryConfig):
    ...
    def get_attribute(self, prediction: Prediction | None, model: BaseModel, dataset: Dataset, generator: Generator,
        ...
        for _ in range(self.num):
            ...
            p_tmp: Prediction = model.predict(data_clone, acquisition=True)
            max_scores = p_tmp.get_max_score(propagated=True)
            proxy += max_scores
        proxy /= self.num
